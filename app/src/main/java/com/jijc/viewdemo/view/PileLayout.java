...
import com.jijc.viewdemo.utils.RoundBitmapUtils;

import java.util.ArrayList;

/**
 * Description:类似于点赞头像排列展示的layout
 * Created by JIJC on 2017/10/24.
 */
...
public class PileLayout extends ViewGroup {
    ...
    public PileLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        vertivalSpace = ta.getDimension(R.styleable.PileLayout_vertical_space, dipToPx(4));
        pileWidth = ta.getDimension(R.styleable.PileLayout_pile_width, dipToPx(10));
        ta.recycle();
    }

    public void initView(Context context, ArrayList<String> imgUrls,int itemResId){
        LayoutInflater inflater = LayoutInflater.from(context);
        for (int i = 0; i< ListUtils.getSize(imgUrls); i++){
            ImageView imageView = (ImageView) inflater.inflate(itemResId,this, false);
            RoundBitmapUtils.roundImageView(imgUrls.get(i),imageView,R.drawable.default_avatar);
